...
show_banner() {
    ...
    echo "  ██║ ╚═╝ ██║   ██║   ██║     ██║  ██║╚██████╔╝   ██║   ╚██████╔╝"
    echo "  ╚═╝     ╚═╝   ╚═╝   ╚═╝     ╚═╝  ╚═╝ ╚═════╝    ╚═╝   ╚═════╝  "
    echo -e "${RESET}${MAGENTA}        MTProto Proxy + Stub Site + Web Panel Installer"
    echo -e "${RESET}"
}
...
# Создаём конфиг для сайта-заглушки
cat > /etc/nginx/sites-available/${MAIN_DOMAIN} << NGINXEOF
server {
    listen 80;
    listen [::]:80;
...
# Создаём конфиг для панели управления
...
if nginx -t >/dev/null 2>&1; then
    ...
    echo -e "${GREEN}Nginx настроен и перезапущен${RESET}"
else
    echo -e "${RED}Ошибка в конфигурации Nginx!${RESET}"
    nginx -t
    exit 1
fi

# ==========================================
# ЧАСТЬ 4: СОЗДАНИЕ САЙТА-ЗАГЛУШКИ
# ==========================================
echo -e "${BOLD}--- СОЗДАНИЕ САЙТА-ЗАГЛУШКИ ---${RESET}"
...
echo -e "${GREEN}Сайт-заглушка создан${RESET}"

# ==========================================
# ЧАСТЬ 5: УСТАНОВКА MTProto ПРОКСИ
# ==========================================
echo -e "${BOLD}--- УСТАНОВКА MTProto ПРОКСИ ---${RESET}"
...
# ==========================================
# ЧАСТЬ 7: УСТАНОВКА WEB UI ПАНЕЛИ
# ==========================================
echo -e "${BOLD}--- УСТАНОВКА WEB UI ПАНЕЛИ ---${RESET}"
...
# Systemd служба для панели (без SSL в gunicorn, SSL терминирует nginx)
echo -e "${YELLOW}Настройка службы панели...${RESET}"
cat > /etc/systemd/system/telemt-panel.service << EOF
[Unit]
Description=MTProto Proxy Web Panel (Gunicorn)
