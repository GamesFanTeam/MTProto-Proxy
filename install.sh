...
# ЧАСТЬ 1: СБОР ВВОДНЫХ ДАННЫХ
# ==========================================
echo -e "${BOLD}--- НАСТРОЙКА ПРОКСИ, САЙТА-ЗАГЛУШКИ И ПАНЕЛИ ---${RESET}"
echo ""

# Домен для САЙТА-ЗАГЛУШКИ (основной домен, порт 443)
read -rp "Введите ОСНОВНОЙ домен для САЙТА-ЗАГЛУШКИ (напр. example.com): " MAIN_DOMAIN
if [[ -z "${MAIN_DOMAIN}" ]]; then
    echo -e "${RED}Основной домен обязателен!${RESET}"
    exit 1
fi

# Домен для ПРОКСИ (поддомен, порт 443)
read -rp "Введите поддомен для ПРОКСИ (напр. tg.example.com): " PROXY_DOMAIN
if [[ -z "${PROXY_DOMAIN}" ]]; then
    echo -e "${RED}Домен для прокси обязателен!${RESET}"
    exit 1
fi

# Домен для ПАНЕЛИ (поддомен)
read -rp "Введите поддомен для ПАНЕЛИ УПРАВЛЕНИЯ (напр. admin.example.com): " PANEL_DOMAIN
if [[ -z "${PANEL_DOMAIN}" ]]; then
    echo -e "${RED}Домен для панели обязателен!${RESET}"
    exit 1
fi
...
echo ""
echo -e "${CYAN}Параметры установки:${RESET}"
echo -e "  ${BLUE}Сайт-заглушка:${RESET} ${MAIN_DOMAIN} (порт 443)"
echo -e "  ${BLUE}Прокси домен:${RESET} ${PROXY_DOMAIN} (порт 443)"
echo -e "  ${BLUE}Панель домен:${RESET} ${PANEL_DOMAIN} (порт ${PANEL_PORT})"
if [[ -n "${CERT_EMAIL}" ]]; then
    echo -e "  ${BLUE}Email для сертификатов:${RESET} ${CERT_EMAIL}"
...
# Сертификат для САЙТА-ЗАГЛУШКИ
echo -ne "${YELLOW}Проверка/выпуск сертификата для ${MAIN_DOMAIN}... ${RESET}"
ssl_main_status=$(issue_ssl "$MAIN_DOMAIN" "$CERT_EMAIL")
case "$ssl_main_status" in
    "exist")
        echo -e "${GREEN}Найден существующий (действителен)${RESET}"
        ;;
    "new")
...
if ufw status >/dev/null 2>&1; then
    ufw --force reload >/dev/null 2>&1
    echo -e "${GREEN}Правила firewall применены${RESET}"
    echo -e "  - Порт 443 (Прокси + Сайт): ${GREEN}открыт${RESET}"
    echo -e "  - Порт 80 (SSL сертификация): ${GREEN}открыт${RESET}"
    echo -e "  - Порт ${PANEL_PORT} (Панель): ${GREEN}открыт${RESET}"
else
    echo -e "${YELLOW}UFW не активен, порты будут открыты при включении${RESET}"
fi
...
# ==========================================
# ЧАСТЬ 5: НАСТРОЙКА NGINX ДЛЯ САЙТА-ЗАГЛУШКИ
# ==========================================
echo -e "${BOLD}--- НАСТРОЙКА NGINX ДЛЯ САЙТА-ЗАГЛУШКИ ---${RESET}"

# Создаем директорию для сайта-заглушки
STUB_DIR="/var/www/stub-site"
mkdir -p "$STUB_DIR"

# HTML код страницы-заглушки
...
/* Адаптивный шрифт: крупнее на телефоне, компактнее на десктопе */
...
<div class="incident" id="incident-code">
Инцидент: генерация...
</div>
...
<a href="mailto:rsoc_in@rkn.gov.ru" class="support-link">
Служба поддержки
</a>
...
document.getElementById('incident-code').innerText =
'Инцидент: ' + generateIncident();
...
echo -e "${GREEN}Файл страницы-заглушки создан${RESET}"

# Конфигурация Nginx для сайта-заглушки (основной домен)
...
# Активируем сайт-заглушку
ln -sf /etc/nginx/sites-available/stub-site /etc/nginx/sites-enabled/stub-site

echo -e "${GREEN}Конфигурация Nginx для сайта-заглушки создана${RESET}"
...
# ==========================================
# ЧАСТЬ 7: ФИНАЛЬНАЯ НАСТРОЙКА NGINX
# ==========================================
echo -e "${BOLD}--- ФИНАЛЬНАЯ НАСТРОЙКА NGINX ---${RESET}"
...
# Перезапускаем nginx
echo -ne "${YELLOW}Перезапуск Nginx... ${RESET}"
...
echo -e "${GREEN}Nginx настроен для обслуживания:${RESET}"
echo -e "  - ${MAIN_DOMAIN}:443 → Сайт-заглушка"
echo -e "  - ${PROXY_DOMAIN}:443 → MTProto Proxy (через SNI)"
echo -e "  - ${PANEL_DOMAIN}:${PANEL_PORT} → Панель управления (Gunicorn)"
echo ""

# ==========================================
# ЧАСТЬ 8: АВТООБНОВЛЕНИЕ
# ==========================================
echo -e "${BOLD}--- НАСТРОЙКА АВТООБНОВЛЕНИЯ ---${RESET}"
...
echo -e "   URL: ${GREEN}https://${MAIN_DOMAIN}${RESET}"
echo -e "   Статус: ${GREEN}Работает через HTTPS${RESET}"
echo ""
echo -e "${BOLD}📡 ПРОКСИ:${RESET}"
echo -e "   Домен: ${GREEN}${PROXY_DOMAIN}${RESET}"
echo -e "   Порт: ${GREEN}443${RESET}"
