...
show_banner() {
    ...
    echo "  ██║ ╚═╝ ██║   ██║   ██║     ██║  ██║╚██████╔╝   ██║   ╚██████╔╝"
    echo "  ╚═╝     ╚═╝   ╚═╝   ╚═╝     ╚═╝  ╚═╝ ╚═════╝    ╚═╝   ╚═════╝  "
    echo -e "${RESET}${MAGENTA}        MTProto Proxy Installer with Stub Site & Web Panel"
    echo -e "${RESET}"
}
...
issue_ssl() {
    ...
    if [[ -f "$cert_path" ]]; then
        # Проверяем действительность сертификата (действителен ли еще 30 дней)
        if openssl x509 -in "$cert_path" -noout -checkend 2592000 >/dev/null 2>&1; then
            echo "exist"
        else
            # Сертификат истекает, перевыпускаем
            if [[ -n "$email" ]]; then
...
# ЧАСТЬ 1: СБОР ВВОДНЫХ ДАННЫХ
# ==========================================
echo -e "${BOLD}--- НАСТРОЙКА ПРОКСИ, САЙТА И ПАНЕЛИ ---${RESET}"
echo ""

# Домен для ПРОКСИ (порт 443)
read -rp "Введите домен для MTProto Proxy (например, tg.example.com): " PROXY_DOMAIN
if [[ -z "${PROXY_DOMAIN}" ]]; then
    echo -e "${RED}Домен для прокси обязателен!${RESET}"
    exit 1
fi

# Домен для САЙТА-ЗАГЛУШКИ (порт 80)
read -rp "Введите домен для Сайта-заглушки (например, example.com): " STUB_DOMAIN
if [[ -z "${STUB_DOMAIN}" ]]; then
    echo -e "${RED}Домен для сайта-заглушки обязателен!${RESET}"
    exit 1
fi

# Домен для ВЕБ-ПАНЕЛИ (порт 4444)
read -rp "Введите домен для Веб-панели (например, admin.example.com): " PANEL_DOMAIN
if [[ -z "${PANEL_DOMAIN}" ]]; then
    echo -e "${RED}Домен для веб-панели обязателен!${RESET}"
    exit 1
fi
...
# Email для сертификатов
read -rp "Email для SSL-сертификатов Let's Encrypt (не обязательно): " CERT_EMAIL

echo ""
echo -e "${CYAN}Параметры установки:${RESET}"
echo -e "  ${BLUE}Прокси домен:${RESET} ${PROXY_DOMAIN} (порт 443, HTTPS)"
echo -e "  ${BLUE}Сайт-заглушка домен:${RESET} ${STUB_DOMAIN} (порт 80, HTTP)"
echo -e "  ${BLUE}Веб-панель домен:${RESET} ${PANEL_DOMAIN} (порт ${PANEL_PORT}, HTTPS)"
if [[ -n "${CERT_EMAIL}" ]]; then
    echo -e "  ${BLUE}Email для сертификатов:${RESET} ${CERT_EMAIL}"
else
    echo -e "  ${YELLOW}Email для сертификатов:${RESET} не указан"
...
case "$ssl_proxy_status" in
    ...
    "renewed")
        ...
        ;;
    "error")
        echo -e "${YELLOW}Предупреждение: выпуск SSL не удался (проверьте A-запись)${RESET}"
        ;;
esac
...
# ==========================================
# ЧАСТЬ 3: УСТАНОВКА MTProto ПРОКСИ
# ==========================================
echo -e "${BOLD}--- УСТАНОВКА MTProto ПРОКСИ ---${RESET}"
...
if ss -tulpen 2>/dev/null | grep -q ":443" || netstat -tulpen 2>/dev/null | grep -q ":443"; then
    ...
    if command -v lsof >/dev/null 2>&1 && lsof -i :443 >/dev/null 2>&1; then
        ...
        echo -e "${YELLOW}Порт 443 не найден в списке слушающих${RESET}"
    fi
fi
...
if ufw status >/dev/null 2>&1; then
    ufw --force reload >/dev/null 2>&1
    echo -e "${GREEN}Правила firewall применены${RESET}"
    echo -e "  - Порт 80 (Сайт-заглушка): ${GREEN}открыт${RESET}"
    echo -e "  - Порт 443 (Прокси): ${GREEN}открыт${RESET}"
    echo -e "  - Порт ${PANEL_PORT} (Панель): ${GREEN}открыт${RESET}"
else
    echo -e "${YELLOW}UFW не активен, порты будут открыты при включении${RESET}"
...
# ==========================================
echo -e "${BOLD}--- НАСТРОЙКА NGINX ---${RESET}"

# Создаем директорию для сайта-заглушки
STUB_DIR="/var/www/stub-site"
mkdir -p "$STUB_DIR"

# HTML страницы заглушки (Роскомнадзор стиль)
...
echo -e "${GREEN}Страница заглушки создана${RESET}"

# Конфигурация Nginx для сайта-заглушки (порт 80)
cat > /etc/nginx/sites-available/stub-site << EOF
server {
    listen 80;
    ...
    # Блокируем доступ к другим доменам на этом сервере
    location ~ ^/(?!index\.html\$) {
        return 404;
    }
}
...
# Конфигурация Nginx для веб-панели (SSL reverse proxy)
PANEL_DIR="/var/www/telemt-panel"
mkdir -p "$PANEL_DIR/templates"

# Проверяем существуют ли SSL сертификаты для панели
...
if [[ -f "$PANEL_SSL_CERT" && -f "$PANEL_SSL_KEY" ]]; then
    # Используем SSL
...
# Включаем сайты
ln -sf /etc/nginx/sites-available/stub-site /etc/nginx/sites-enabled/
ln -sf /etc/nginx/sites-available/telemt-panel /etc/nginx/sites-enabled/
rm -f /etc/nginx/sites-enabled/default

# Тест и перезапуск Nginx
echo -ne "${YELLOW}Проверка конфигурации Nginx... ${RESET}"
if nginx -t >/dev/null 2>&1; then
    echo -e "${GREEN}OK${RESET}"
    systemctl restart nginx
    echo -e "${GREEN}Nginx перезапущен${RESET}"
else
    echo -e "${RED}ОШИБКА конфигурации Nginx${RESET}"
    nginx -t 2>&1 | head -5
fi
echo ""

# ==========================================
# ЧАСТЬ 6: УСТАНОВКА WEB UI ПАНЕЛИ
# ==========================================
echo -e "${BOLD}--- УСТАНОВКА WEB UI ПАНЕЛИ ---${RESET}"
...
# Systemd служба для панели (без SSL в Gunicorn, SSL обрабатывает Nginx)
echo -e "${YELLOW}Настройка службы панели...${RESET}"
cat > /etc/systemd/system/telemt-panel.service << EOF
[Unit]
Description=MTProto Proxy Web Panel (Gunicorn)
...
Environment="PATH=${PANEL_DIR}/venv/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
Environment="PANEL_PORT=${PANEL_PORT}"
ExecStart=${PANEL_DIR}/venv/bin/gunicorn -w 2 -b 127.0.0.1:${PANEL_PORT} app:app
Restart=always
RestartSec=3
...
(crontab -l 2>/dev/null | grep -v "telemt-updater" | grep -v "certbot renew") | crontab - 2>/dev/null || true
(crontab -l 2>/dev/null 2>/dev/null; echo "0 4 * * * /usr/local/bin/telemt-updater.sh") | crontab - 2>/dev/null || true
(crontab -l 2>/dev/null 2>/dev/null; echo "0 3 * * * certbot renew --post-hook 'systemctl restart telemt telemt-panel nginx' >/dev/null 2>&1") | crontab - 2>/dev/null || true
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo -e "${RESET}"
echo -e "${BOLD}📡 MTProto PROXY:${RESET}"
echo -e "   Домен: ${GREEN}${PROXY_DOMAIN}${RESET}"
echo -e "   Порт: ${GREEN}443${RESET}"
echo -e "   Протокол: ${GREEN}HTTPS (Fake TLS)${RESET}"
echo -e "   Маскировка: ${YELLOW}${FAKE_DOMAIN}${RESET}"
echo ""
echo -e "${BOLD}🌐 САЙТ-ЗАГЛУШКА:${RESET}"
echo -e "   Домен: ${GREEN}${STUB_DOMAIN}${RESET}"
echo -e "   Порт: ${GREEN}80${RESET}"
echo -e "   Протокол: ${GREEN}HTTP${RESET}"
echo ""
echo -e "${BOLD}🖥️ ВЕБ-ПАНЕЛЬ УПРАВЛЕНИЯ:${RESET}"
echo -e "   Домен: ${GREEN}${PANEL_DOMAIN}${RESET}"
echo -e "   Порт: ${GREEN}${PANEL_PORT}${RESET}"
if [[ -f "$PANEL_SSL_CERT" && -f "$PANEL_SSL_KEY" ]]; then
    echo -e "   Протокол: ${GREEN}HTTPS (SSL)${RESET}"
else
    echo -e "   Протокол: ${YELLOW}HTTP (SSL не настроен)${RESET}"
fi
echo -e "   Логин: ${YELLOW}admin${RESET}"
echo -e "   Пароль: ${YELLOW}admin${RESET}"
echo -e "   ${RED}⚠️ Смените пароль при первом входе!${RESET}"
echo ""
echo -e "${BOLD}🔗 ССЫЛКА ДЛЯ ПОДКЛЮЧЕНИЯ К ПРОКСИ:${RESET}"
echo -e "${GREEN}${TG_LINK}${RESET}"
echo ""
echo -e "${BOLD}🔗 URL ВЕБ-ПАНЕЛИ:${RESET}"
if [[ -f "$PANEL_SSL_CERT" && -f "$PANEL_SSL_KEY" ]]; then
    echo -e "${GREEN}https://${PANEL_DOMAIN}:${PANEL_PORT}${RESET}"
else
    echo -e "${YELLOW}http://${PANEL_DOMAIN}:${PANEL_PORT}${RESET}"
fi
echo ""
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo ""
